...
describe('Use cache headers', function() {
  it('We should be able to know if cache times are too short', function() {
    return har.firstAdviceForTestFile('cacheHeaders.har').then((result) => {
      assert.strictEqual(result.performance.adviceList.cacheHeadersLong.score, 68);
      // massive
      assert.strictEqual(result.performance.adviceList.cacheHeadersLong.offending.length, 32);
    });
  });
