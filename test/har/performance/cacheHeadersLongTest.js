...
describe('Use cache headers', function() {
  it('We should be able to know if cache times are too short', function() {
    return har.getHARresult('test/har/files/cacheHeaders.har').then((result) => {
      assert.strictEqual(result[0].coachAdvice.results.performance.adviceList.cacheHeadersLong.score, 19);
      // massive
      assert.strictEqual(result[0].coachAdvice.results.performance.adviceList.cacheHeadersLong.offending.length, 81);
    });
  });
