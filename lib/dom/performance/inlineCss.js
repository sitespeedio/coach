(function() {
  ...
  if (util.isHTTP2() && cssFilesInHead.length > 0) {
    message = 'Make sure you are using server push for your CSS files (when your server supports push). If you have many users on slow connections, it can be better to inline the CSS file(s). Make sure to run your own tests and check a waterfall graph to see what happens.';
  }

  // If we got inline styles with HTTP/2 recommend that we push CSS responses instead
  if (util.isHTTP2() && styles.length > 0) {
    score += 5;
    message += 'The page inlince CSS and uses HTTP/2. Do you have a lot of users with slow connections on the site, it could be good to inline CSS when using HTTP/2. If not and your server supports server push, use it to push the CSS files instead.'
  }
  ...
  if (!util.isHTTP2()) {
    ...
    if (styles.length > 0 && cssFilesInHead.length > 0) {
      score += 10;
      message += 'The page has both inline styles as well as it is requesting CSS files inside of head. Let\'s only inline CSS for really fast render.';
      offending.push.apply(offending, cssFilesInHead);
    }
  }
