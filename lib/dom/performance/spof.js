(function(util) {
  ...
  return {
    ...
    description: 'A page should not have a single point of failure. For example; loading content from a provider that can cause the page to stop working.',
    advice: offending.length > 0 ? 'The page has ' + offending.length + ' requests inside of head that can cause a SPOF. Load them asynchronously or move them outside of document head.' : '',
