  ...
  if (util.isHTTP2()) {
    if (styles.length > 0) {
      ...
      styles.forEach(function(style) {
        var url = util.getAbsoluteURL(style);
    ...
    if (scripts.length > 0) {
      ...
      offending.push(util.getAbsoluteURL(script));
      });
      blockingJS++;
    ...
    if (offending.length > 0) {
      message += 'The page has ' + blockingCSS + ' render blocking CSS request(s) and ' + blockingJS + ' Javascript request(s) inside of head.';
    }
