/**
 * The sitespeed.io coach (https://www.sitespeed.io/coach)
 * Copyright (c) 2016, Peter Hedenskog, Tobias Lidskog
 * and other contributors
 * Released under the Apache 2.0 License
 */
(function() {
  'use strict';
  var score = 100;
  var message = '';
  var docType = document.doctype;
  if (docType === null) {
    message = 'The page is missing a doc type. Please use <!DOCTYPE html>';
    score = 0;
  }
  else if (document.firstChild !== '<!DOCTYPE html>') {
    message = 'You should have the the doctype declaration first in your HTML file and you should declare using HTML5 standard: <!DOCTYPE html>';
    score = 50;
  }


  return {
    id: 'doctype',
    title: 'Use a doctype for your document',
    description: 'The <!DOCTYPE> declaration is not an HTML tag; it is an instruction to the web browser about what version of HTML the page is written in.',
    advice: message,
    score: score,
    weight: 2,
    offending: [],
    tags: ['bestpractice']
  };
})();
