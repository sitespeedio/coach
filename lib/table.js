  ...
  setupAdviceTable(type) {
    ...
    Object.keys(adviceList).forEach(function(advice) {
      ...
      if (score < self.options.limit) {
        self.table.push([getColoredMessage(score, advice), adviceList[advice].advice.replace(/(.{53})/g, "$1" + EOL), getColoredMessage(score)]);

        // show offendinf if it's configured and we have some assets
