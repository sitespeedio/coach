...
module.exports = {
  ...
  processPage: function(page) {
    ...
    return {
      ...
      offending: offending,
      advice: score < 100 ? 'The page has ' + offending.length + ' redirect(s). ' +  (sameDomainAsDocument > 0 ? sameDomainAsDocument + ' of the redirects are from the base domain, please fix them! ' : '') + (offending.length > sameDomainAsDocument ? (offending.length - sameDomainAsDocument) + ' request(s) are from other domains, it could be 3rd party assets that do redirects that they really don\'t need :(' : ''): ''
    }
  }
};
