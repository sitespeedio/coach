...
function processAsset(asset) {
  if (SKIPPABLE_DOMAINS.indexOf(util.getHostname(asset.url)) > -1) {
    ...
    // this is catched in cacheHeaders so lets skip giving advice
    // about them
    return 0;
  }
  else {
...
module.exports = {
  ...
  processPage: function(page) {
    ...
    return {
      ...
      offending: offending,
      advice: score < 100 ? 'The page has ' + offending.length + ' request(s) that have a shorter cache time than 1 month (but still a cache time). Can you increase them?' : ''
    }
  }
};
