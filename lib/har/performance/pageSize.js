  ...
  processPage: function(page, domAdvice, options) {
    ...
    if (page.transferSize > sizeLimit) {
      return {
        score: 0,
        offending: [],
        advice: 'The page total transfer size is ' + util.formatBytes(page.transferSize) + ', which is more than the coach limit of ' + util.formatBytes(sizeLimit) + '. That is ' + (page.transferSize > 5000000 ? 'insane' : 'really big') +  ' and you need to make it smaller.'
      };
    }
    return {
      score: 100,
