...
module.exports = {
  ...
  processPage: function(page) {
    let score = 100;
    let advice = '';
    // if a HTTP redirects to HTTPS don't hurt that becasue that is
    // really nice :)
    if (page.documentRedirects === 1 && page.url !== page.finalUrl) {
       // do we redirect to a HTTP page?
       if (page.url.indexOf('http:')> -1 && page.finalUrl.indexOf('https')) {
         advice = 'The page redirect from HTTP to HTTPS, that\s good!';
       }
    } else {
      score = page.documentRedirects > 0 ? 0 : 100;
      advice =  page.documentRedirects > 0 ? 'The main document gets redirected ' + page.documentRedirects + ' time(s). Remove those redirect and make the page faster!' : '';
    }
