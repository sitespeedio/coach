  ...
  processPage: function(page) {
    ...
    page.assets.forEach(function(asset) {
      ...
      if (myScore > 0 && page.url !== asset.url) {
        ...
        offending.push(asset.url);
        advice = 'The main page has a private header. It could be right in some cases where the user can be logged in and served specific content. But if your is static it should never be private. ';
      }
    ...
    return {
      ...
      offending: offending,
      advice: score < 100 ? 'The page has ' + offending.length + ' request(s) with private headers. ' + advice + 'Make sure that the assets really should be private and only used by one user. Else make it cacheable for everyone.' : advice
    }
  }
};
