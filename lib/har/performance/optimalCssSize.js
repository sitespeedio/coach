  ...
  processPage: function(page) {
    ...
    cssAssets.forEach(function(asset) {
      if (asset.transferSize > transferLimit) {
        ...
        advice += asset.url + ' size is ' + util.formatBytes(asset.transferSize) + ' and that is bigger than the limit of ' + util.formatBytes(asset.transferLimit) + '. '
      }
    });

    if (score < 100) {
      advice += 'Try to make the CSS files fit into 14 kb.';
    }
