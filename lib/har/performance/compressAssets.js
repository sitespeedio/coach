...
function processAsset(asset) {
  if (types.indexOf(asset.type) > -1) {
    ...
    if (encoding !== 'gzip' && encoding !== 'deflate' && asset.contentSize > 2000) {
      return 10;
    }
  }
  return 0;
...
module.exports = {
  ...
  processPage: function(page) {
    ...
    return {
      ...
      offending: offending,
      advice: score < 100 ? 'The page has ' + offending.length + ' requests that are served uncompressed. You could save a lot of bytes by sending them compressed instead.' : ''
    }
  }
};
