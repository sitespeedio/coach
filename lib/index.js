...
function getPagesFromHar(harJson) {
  return Promise.resolve(harJson)
    .then((harJson) => pagexray.convert(harJson, {
      includeAssets: true
    }));
}
...
function runBrowsertime(url, script, options) {
  ...
  const runner = new browsertime.Engine(options);

  return runner.start()
    .then(() => runner.run(url, browsertimeify(script)))
    .finally(() => runner.stop());
}
...
module.exports = {
  ...
  runDomAdvice(url, script, options) {
    ...
    return runBrowsertime(url, script, options)
      .then((result) => {
        if (options.screenshot) {
          result.browserScripts[0].coach.domAdvice.screenshot = result.screenshots[0];
        }
  ...
  runHarAdvice(har, script, domAdvice, options) {
    ...
    return harRunner.runAdvice(getPagesFromHar(har), script, domAdvice, options);
  },
  ...
  run(url, domScript, harScript, options) {
    ...
    options = merge({}, DEFAULT_OPTIONS, options);
    return runBrowsertime(url, domScript, options)
      .then((result) => {
        ...
        return this.runHarAdvice(har, harScript, domAdvice, options)
          .then((harAdvice) => this.merge(domAdvice, harAdvice))
      });
  }
};
