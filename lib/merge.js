...
function recalculateScore(results) {
  ...
  Object.keys(results.coachAdvice.results).forEach(function(result) {
    const category = results.coachAdvice.results[result];
    var categoryScore = 0,
      categoryWeight = 0,
  ...
  results.coachAdvice.results.score = Math.round(totalScore / totalWeight);
  return results;
}
